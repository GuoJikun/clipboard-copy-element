 ...
 */
function addEvents(contentMenu: HTMLElement, hoverStyle: HoverStyle, fn) {
  contentMenu.addEventListener("click", (ev: any) => {
  ...
  contentMenu.addEventListener("mouseover", (ev: any) => {
    const target = ev.target;
    const nodeName = target.nodeName.toLowerCase();
    if (nodeName === "li") {
      const brothers = findBrothersElement(target);
      brothers.map((ele: HTMLElement) => {
...
function createRootEle(point) {
  ...
  contentMenu.style.top = point.y ? point.y + "px" : "0px";
  contentMenu.style.left = point.x ? point.x + "px" : "0px";
 ...
 * @param {*} style 样式
 */
function applyCustomStyleToRoot(root: HTMLElement, style: StyleSheet) {
...
function createMenu(
  arr: Array<{ command: string; name: string }>,
  hoverStyle?: HoverStyle
) {
  const menuList = arr.map((item, i) => {
    let command: string | number = i;
    let name = "";
    if (item.command) {
